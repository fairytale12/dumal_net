...
<?if(!empty($arResult['ID'])):?>
	<?$arElement = $GLOBALS['DB']->query('SELECT `SHOW_COUNTER` COUNTER FROM `b_iblock_element` WHERE `ID` = ' . $arResult['ID'])->fetch();?>
	<?itc\CUncachedArea::startCapture('article-counter-block');?>
		<span><i class="fa fa-eye"></i> <?=ft\CHelper::wordDeclension($arElement['COUNTER'], 'просмотр', 'просмотра', 'просмотров')?></span>
	<?itc\CUncachedArea::endCapture();?>
<?endif;?>